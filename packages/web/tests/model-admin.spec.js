...
test("superuser connects a managed provider without exposing model editing", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Models", exact: true }).locator("svg.lucide-cpu")).toBeVisible();
  ...
  await expect(picker.getByRole("button", { name: "Custom OpenAI / Anthropic" })).toBeVisible();
  ...
  await expect(page.locator(".workspaceModelsKeyInput")).toHaveCSS("border-top-color", "rgb(36, 105, 199)");
  ...
  const disconnectDialog = page.getByRole("dialog", { name: "Disconnect provider?" });
  await expect(disconnectDialog).toContainText("“Moonshot AI CN” and its models will no longer be available.");
...
test("custom provider renames explicitly and model ID updates the tree before save", async ({ page }) => {
  ...
  await page.getByLabel("Provider name", { exact: true }).fill("vLLM");
...
test("saved provider rename persists immediately via PATCH", async ({ page }) => {
  ...
  await page.getByLabel("Provider name", { exact: true }).fill("Qwen vLLM");
  await page.getByRole("button", { name: "Rename", exact: true }).click();
  await expect(page.getByText("Saved", { exact: true })).toBeVisible();
...
test("refreshes the retained chat model picker and ignores superseded model responses", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "AI model", exact: true })).toContainText("Kimi K3");
  await page.getByRole("button", { name: "Default workspace menu" }).click();
  await page.getByRole("link", { name: "Settings", exact: true }).click();
  await page.getByRole("dialog", { name: "Preferences", exact: true }).getByRole("link", { name: "Models", exact: true }).click();
  await page.getByRole("button", { name: "Disconnect", exact: true }).click();
  await page.getByRole("dialog", { name: "Disconnect provider?" }).getByRole("button", { name: "Disconnect", exact: true }).click();
  await expect(page.getByRole("dialog", { name: "Disconnect provider?" })).toHaveCount(0);
  ...
  await expect(page.getByRole("button", { name: "AI model", exact: true })).toHaveText("Not configured");
  releaseStale();
  await staleDelivery;
