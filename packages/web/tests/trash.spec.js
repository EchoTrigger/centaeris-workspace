...
test("trash groups current Workspace data with the global private Library", async ({ page }) => {
  ...
  await expect(page.getByText("Items stay in trash for 30 days before being deleted automatically.", { exact: true })).toBeVisible();
  ...
  await page.getByLabel("Type").selectOption("library");
  await expect(page.getByText("私人资料.md", { exact: true })).toBeVisible();
  await expect(page.getByText("Deleted Source", { exact: true })).toHaveCount(0);
  await page.getByLabel("Type").selectOption("");
  await page.getByLabel("Search trash").fill("Deleted Source");
  await expect(page.getByText("Deleted Source", { exact: true })).toBeVisible();
  await expect(page.getByText("私人资料.md", { exact: true })).toHaveCount(0);
  await page.getByLabel("Search trash").fill("");
  await expect(page.getByText("Deleted Agent", { exact: true })).toBeVisible();
  await page.locator(".shTrashRow").filter({ hasText: "私人资料.md" }).getByRole("button", { name: "Permanently delete 私人资料.md" }).click();
  await expect(page.getByRole("dialog", { name: "Delete this material?" })).toBeVisible();
  await page.getByRole("dialog", { name: "Delete this material?" }).getByRole("button", { name: "Cancel", exact: true }).click();
  await page.locator(".shTrashRow").filter({ hasText: "Older Agent" }).getByRole("button", { name: "Permanently delete Older Agent" }).click();
  await expect(page.getByRole("dialog", { name: "Delete this agent and its conversations?" })).toBeVisible();
  await page.getByRole("dialog", { name: "Delete this agent and its conversations?" }).getByRole("button", { name: "Cancel", exact: true }).click();
  ...
  await page.locator(".shTrashRow").filter({ hasText: deletedSession.title }).getByRole("button", { name: `Permanently delete ${deletedSession.title}` }).click();
  const sessionDialog = page.getByRole("dialog", { name: "Delete this conversation?" });
  await expect(sessionDialog).toBeVisible();
  await expect(sessionDialog.locator("p")).toHaveCount(0);
  await expect(sessionDialog.getByRole("button", { name: "Cancel", exact: true })).toBeVisible();
  await expect(sessionDialog.getByRole("button", { name: "Confirm", exact: true })).toBeVisible();
  await sessionDialog.getByRole("button", { name: "Cancel", exact: true }).click();
  expect(purgeRequests).toEqual([]);

  await page.locator(".shTrashRow").filter({ hasText: "Deleted Source" }).getByRole("button", { name: "Permanently delete Deleted Source" }).click();
  const sourceDialog = page.getByRole("dialog", { name: "Delete this source?" });
  await expect(sourceDialog).toBeVisible();
  await expect(sourceDialog.locator("p")).toHaveCount(0);
  await sourceDialog.getByRole("button", { name: "Confirm", exact: true }).click();
  await expect(page.getByText("Deleted Source", { exact: true })).toHaveCount(0);
  ...
  await expect(page.getByText(/This conversation is in trash, with \d+ days remaining./)).toBeVisible();
  await expect(page.getByRole("textbox", { name: "Message" })).toHaveCount(0);
  await page.getByRole("button", { name: "Delete permanently", exact: true }).click();
  const routeDialog = page.getByRole("dialog", { name: "Delete this conversation?" });
  await expect(routeDialog).toBeVisible();
  await expect(routeDialog.locator("p")).toHaveCount(0);
  await routeDialog.getByRole("button", { name: "Cancel", exact: true }).click();
  expect(purgeRequests).toEqual(["/api/workspaces/ws_1/sources/source_deleted/trash"]);
